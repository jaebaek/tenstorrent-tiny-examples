...
void kernel_main() {
  uint32_t input0_dram_addr = get_arg_val<uint32_t>(0);
  uint32_t input1_dram_addr = get_arg_val<uint32_t>(1);
  ...
  bank_for_input0.noc_async_read_tile(0, L1_write_addr_in0);
  ...
  bank_for_input1.noc_async_read_tile(0, L1_write_addr_in1);
  noc_async_read_barrier();
